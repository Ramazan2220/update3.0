...
def handle_add_user_data(update: Update, context: CallbackContext, text: str):
    ...
    try:
        ...
        # КРИТИЧЕСКИ ВАЖНО: Принудительная синхронизация доступов
        try:
            force_sync_access()
...
def handle_delete_user_execute(update: Update, context: CallbackContext):
    ...
    # Полностью удаляем пользователя из системы (включая кеш и принудительную синхронизацию)
    success = delete_user_completely(target_user_id)

...
from database.db_manager import (
    get_session,
    create_publish_task, get_user_active_accounts, get_proxies, save_media_file,
...
# Глобальные селекторы аккаунтов для публикации
story_selector = None
reels_selector = None

def initialize_selectors():
    """Инициализация глобальных селекторов"""
    global story_selector, reels_selector
...
def publish_now_handler(update, context):
    ...
    return publish_selector.start_selection(update, context, on_accounts_selected)
...
def start_story_publish(update, context):
    ...
    story_selector.start_selection(update, context, on_accounts_selected)
    return ConversationHandler.END
...
def start_igtv_publish(update, context):
    ...
    # Запускаем процесс выбора
    return igtv_selector.start_selection(update, context, on_accounts_selected)
...
def execute_publish_task(update, context):
    ...
    try:
        ...
        for account_id in selected_accounts:
            ...
            # Проверяем лимиты через rate limiter
            action_type = ActionType.POST if task_type in [TaskType.PHOTO, TaskType.VIDEO, TaskType.CAROUSEL] else ActionType.STORY
            can_publish, reason = rate_limiter.can_perform_action(account_id, action_type)
            
            if not can_publish:
                logger.warning(f"❌ Пропускаем {account.username}: {reason}")
                continue
            
            # Проверяем статус аккаунта через automation service
            account_status = automation_service.get_account_status(account_id)
            if account_status.get('ban_risk_score', 0) > 70:
                logger.warning(f"❌ Пропускаем {account.username}: высокий риск бана ({account_status['ban_risk_score']})")
                continue
            ...
            if success and task_id:
                ...
                # Записываем действие в rate limiter
